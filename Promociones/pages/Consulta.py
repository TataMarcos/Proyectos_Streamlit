...
with st.spinner('Consultando en Snowflake...'):
    try:
        cursor.execute(f'''
SELECT 
    MONTH(FT.TIEM_DIA_ID) as MES, LG.GEOG_LOCL_COD AS LOCAL, LAA.ORIN AS ITEM, LAA.ARTC_ARTC_DESC, ROUND(SUM(FT.VNTA_IMPORTE_SIN_IVA), 2) AS VENTA,
    ROUND(SUM(FT.VNTA_UNIDADES), 2) AS VENTA_UNID, ROUND(SUM(FT.VNTA_UNIDADES * COALESCE(FT.VNTA_COSTO_PROM_POND, 0)), 2) AS COSTO, ROUND(VENTA - COSTO, 2) AS GB1,
    ROUND(DIV0(GB1, VENTA), 4) AS MARGEN, ROUND(VENTA - GB1, 2) AS CPP_VENDIDO, ROUND(FS.STCK_UNIDADES * FC.UNIT_COST, 2) AS CPP_ACTUAL, LA.ARTC_ESTA_DESC
FROM
