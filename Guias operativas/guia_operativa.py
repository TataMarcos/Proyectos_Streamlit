...
if 'flujo' not in st.session_state:
    ...
    WITH DIM AS (
    SELECT
        ITEM, MAX(SUPP_PACK_SIZE) AS SUPP_PACK_SIZE, MAX(TI) TI, MAX(HI) HI,
        MEDIAN(TOTAL_UNID) AS MEDIAN_UNID_POR_PALLET,
        ROUND(MEDIAN_UNID_POR_PALLET * 0.25) AS UNIDADES_POR_CUARTO_DE_PALLET
    FROM
        (
        SELECT
            DISTINCT ITEM, SUPPLIER, SUPP_PACK_SIZE, TI, HI, ROUND(SUPP_PACK_SIZE * TI * HI) AS TOTAL_UNID,
            ROUND(TOTAL_UNID * 0.25) AS MINIMO_COMPRA, ROW_NUMBER() OVER(PARTITION BY ITEM ORDER BY SUPP_PACK_SIZE DESC) AS RN
        FROM
    ...
    SELECT
        GUIA_OPERATIVA, B.ARTC_ARTC_COD, B.ORIN, B.ARTC_ARTC_ID, B.ARTC_ARTC_DESC, B.GEOG_LOCL_COD,
        B.GEOG_LOCL_ID, UNIDADES_CARGA, MEDIAN_UNID_POR_PALLET, F.FORMATO, SUM(VENTA_BASAL) AS VENTA_BASAL
    FROM
        (
        SELECT
            DISTINCT MPH.ARTC_ARTC_COD, MPH.ARTC_ARTC_ID, MPH.ARTC_ARTC_DESC, MPH.GEOG_LOCL_COD, MPH.GEOG_LOCL_ID,
            MPH.ORIN, CASE WHEN ORIN IN ({orines_snow}) THEN {estibas_snow} END AS GUIA_OPERATIVA,
            COUNT(MPH.ORIN) OVER(PARTITION BY MPH.GEOG_LOCL_COD, GUIA_OPERATIVA) AS ITEMS_POR_ESTIBA,
            1 / ITEMS_POR_ESTIBA AS PARTICIPACION, ROUND(DIM.MEDIAN_UNID_POR_PALLET, 0) AS MEDIAN_UNID_POR_PALLET,
            CASE WHEN MPH.ARTC_ARTC_COD IN ('0') THEN 0 ELSE PARTICIPACION END AS PARTICIPACION_AJUSTADA,
            ROUND(DIM.UNIDADES_POR_CUARTO_DE_PALLET * PARTICIPACION_AJUSTADA + COALESCE(MPH.CARGA_MAX_FINAL, 0), 0) AS UNIDADES_CARGA
        FROM
            SANDBOX_PLUS.DWH.MAESTRO_PRODUCTOS_HIST AS MPH
        LEFT JOIN
            DIM
        ON MPH.ORIN = DIM.ITEM
        LEFT JOIN
            SANDBOX_PLUS.DWH.ESTIBAS_POR_LOCAL AS EPL
        ON EPL.LOCAL = MPH.GEOG_LOCL_COD
        WHERE
            MONTH(MPH.MES) = MONTH(CURRENT_DATE - 1) AND YEAR(MPH.MES) = YEAR(CURRENT_DATE - 1)
            AND MPH.ESTADO IN ('Activo', 'Discontinuado') AND MPH.ORIN IN ({orines_snow})
            AND GUIA_OPERATIVA <= EPL.ESTIBAS AND EPL.ESTIBAS > 0
        ) B
    JOIN
    ...
    print(df['EXHIBICION DATA'].sum())

    df.loc[(df['PALLET ESPECIAL']=='SI') & (df['FORMATO']=='HIPER'), 'EXHIBICION DATA'] *= 4

    df.loc[(df['LOCAL']!='167') & (df['PALLET ESPECIAL']=='SI') & (df['FORMATO']=='SUPER'),
        'EXHIBICION DATA'] *= 2
